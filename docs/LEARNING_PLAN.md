...
- [x] **阶段一结束（M2）**：能一键下载三个市场的数据并做 EDA。
- [x] **阶段二前段（M3）**：掌握时序建模基础与底层技术指标构建。
- [ ] **阶段二结束（M4）**：能独立做完一次单因子测试（IC、分组、多空、换手）。
- [x] **阶段三结束（M6）**：自研回测引擎跑通 3 个策略（动量/双均线/风险平价），统一交付
  IS/OOS + Walk-Forward + DSR + 参数敏感性 + 归因；与 vectorbt 偏差 < 3%（M5 已验，0.44%）。
  注：momentum 因离线无标普横截面跑在合成面板上（含"零假设 DSR≈0 / 植入真边 DSR≈1"对照）。
