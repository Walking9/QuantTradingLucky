#!/usr/bin/env python
"""Run the ``dual_ma_a_daily_vol_filter`` backtest end-to-end.
...
real A-share basket, runs the full research bundle (full-sample backtest +
IS/OOS split + benchmark attribution + Walk-Forward/DSR + parameter
sensitivity) via :func:`quant_lucky.strategies.run_research`, prints a
summary, and writes reproducible artifacts to ``reports/``.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="KEY=VALUE",
        help="dotted-key config override, e.g. signal.vol_max=null",
    )
